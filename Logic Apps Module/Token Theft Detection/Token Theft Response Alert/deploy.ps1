# deploy.ps1 — Deploy Token Theft Response Alert Logic App
# Developer: Dr Muataz Awad
#
# What this deploys:
#   A Logic App that polls Entra ID Protection every N minutes for token theft
#   risk detections (riskEventType: anomalousToken, tokenIssuerAnomaly). When
#   detections are found, it immediately sends an HTML alert email with affected
#   accounts, detection type, source IP, and token revocation guidance.
...
param(
    ...
    [string]$LogicAppName            = "token-theft-response-alert",
    [int]$PollingIntervalMinutes     = 15                   # Polling interval in minutes (5–60). Default: 15
)

$ErrorActionPreference = "Stop"

Write-Host "=== Token Theft Response Alert — Deploy ===" -ForegroundColor Cyan
...
# Permissions needed:
#   IdentityRiskEvent.Read.All  — read anomalousToken and tokenIssuerAnomaly detections
#   AuditLog.Read.All           — read sign-in logs for session investigation context
...
Write-Host "  2. If no token theft detections exist yet, the run completes silently (no email)."
Write-Host "     Token theft detections (anomalousToken, tokenIssuerAnomaly) are generated by"
Write-Host "     Entra ID Protection when anomalous token replay is identified."
