# Entra Sample Prompts

Ready-to-use sample prompts for common Entra investigation and triage scenarios.

**Developer**: Dr Muataz Awad

## Prompt Samples By Scenario Family

This index separates prompt samples by Microsoft Entra scenario family so each area has dedicated prompts.

- [Entra ID Prompt Samples](Entra%20ID%20Prompt%20Samples.md)
- [Entra ID Protection Prompt Samples](Entra%20ID%20Protection%20Prompt%20Samples.md)
- [Entra ID Governance Prompt Samples](Entra%20ID%20Governance%20Prompt%20Samples.md)
- [Entra Internet and Private Access Prompt Samples](Entra%20Internet%20and%20Private%20Access%20Prompt%20Samples.md)

## Usage Order

1. Open the scenario-family file that matches your investigation.
2. Start with discovery prompts, then drill into analysis prompts.
3. Use role activation/deactivation prompts when elevated access is required.
