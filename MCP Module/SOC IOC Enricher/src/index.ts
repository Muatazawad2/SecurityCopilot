...
type IocType = "ip" | "hash" | "domain" | "unknown";
...
async function enrichIp(ip: string): Promise<object> {
  ...
  // Derive verdict
  const abuseScore = (result.abuse_intelligence as Record<string, number> | undefined)
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.query_status === "hash_found") {
        const sample = (data.data as Record<string, unknown>[])[0];
        result.malware_intelligence = {
          malware_name: sample.signature ?? "Unknown",
          tags: (sample.tags as string[] | null)?.join(", ") ?? "None",
    ...
    if (res.ok) {
      ...
      if (results && results.length > 0) {
        const maliciousCount = results.filter(
          (r) => (r.verdicts as Record<string, Record<string, boolean>>)?.overall?.malicious
        ).length;
        const latest = results[0];
        const page = latest.page as Record<string, string> | undefined;
        const verdicts = latest.verdicts as Record<string, Record<string, unknown>> | undefined;
        ...
        result.urlscan_intelligence = {
          ...
          categories: (verdicts?.overall?.categories as string[] | undefined)?.join(", ") ?? "None",
        };
    ...
    tools: [
      {
        name: "enrich_ioc",
        description:
          "Auto-detect the type of an IOC (IP address, file hash MD5/SHA1/SHA256, or domain) and query multiple threat intelligence sources simultaneously. Returns a single correlated verdict with confidence score and recommended action. This is the primary tool — use this first.",
        inputSchema: {
          type: "object",
          properties: {
            ioc: {
              type: "string",
              description:
                "The indicator of compromise to investigate. Can be an IPv4 address (e.g. 45.33.32.156), a file hash (MD5, SHA1, or SHA256), or a domain name (e.g. malicious-domain.com).",
            },
          },
          required: ["ioc"],
        },
      },
      {
        name: "enrich_ip",
        description:
          "Enrich a specific IP address by querying IPinfo.io for geolocation and ASN data, and optionally AbuseIPDB for abuse confidence score (requires ABUSEIPDB_API_KEY environment variable).",
        inputSchema: {
          type: "object",
          properties: {
            ip: {
      ...
      },
      {
        name: "enrich_hash",
        description:
          "Look up a file hash (MD5, SHA1, or SHA256) in MalwareBazaar to determine if it is associated with known malware. Returns malware family, tags, file type, and first/last seen dates.",
        inputSchema: {
          type: "object",
          properties: {
            hash: {
              type: "string",
              description:
                "File hash to look up. Accepts MD5 (32 chars), SHA1 (40 chars), or SHA256 (64 chars) hex strings.",
            },
          },
          required: ["hash"],
        },
      },
      {
        name: "enrich_domain",
        description:
          "Investigate a domain name by checking DNS resolution (Google Public DNS) and searching URLScan.io for scan history and malicious verdicts.",
        inputSchema: {
          type: "object",
          properties: {
            domain: {
              type: "string",
              description:
                "Domain name to investigate (e.g. malicious-domain.com). Do not include http:// or paths.",
            },
          },
          required: ["domain"],
        },
